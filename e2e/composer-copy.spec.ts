 ...
 * contenteditable (the rich composer) makes Ctrl/Cmd+A collapse to an empty
 * caret when focus is outside the editable, so Ctrl/Cmd+C copies nothing.
 ...
 */
test.describe('composer Ctrl/Cmd+A + Ctrl/Cmd+C copy', () => {
    ...
    test('Ctrl/Cmd+A outside the composer selects the message thread and Ctrl/Cmd+C copies it', async ({ page }) => {
    ...
    test('Ctrl/Cmd+A with composer focused still selects only the composer draft', async ({ page }) => {
    ...
    test('Ctrl/Cmd+A in a textarea keeps native behavior', async ({ page }) => {
        await page.evaluate(() => {
    ...
    test('mouse-drag selection still copies via plain Ctrl/Cmd+C', async ({ page }) => {
